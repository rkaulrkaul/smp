 ...
 * What I thought about this assignment: Learned a lot.
 *
 *
 */
...
public class Heap<E> {
	 ...
	 * The values in b[size..] may or may not be null. DO NOT RELY ON THEM BEING
	 * NULL. <br>
	...
	void swap(int i, int j) {
		...
		// if value v is in b[k], then map.get(v) returns k.
		VP temp = b[i];
		b[i] = b[j];
		b[j] = temp;
		map.put(b[i].val, i);
	...
	void bubbleDown(int k) {
		...
		if (k > size - 1)
			return;
		while (2 * k + 1 < size) {
			if (2 * k + 2 < size && compareTo(2 * k + 1, 2 * k + 2) <= 0 && compareTo(k, 2 * k + 2) < 0) {
				swap(k, 2 * k + 2);
				k = 2 * k + 2;
			} else if (compareTo(k, 2 * k + 1) < 0) {
				swap(k, 2 * k + 1);
				k = 2 * k + 1;
			} else
				return;
	...
	public void changePriority(E v, double p) {
		...
		int k = map.get(v);
		b[k].priority = p;
		if ((k - 1) / 2 >= 0 && compareTo(k, (k - 1) / 2) == 1) {
			bubbleUp(k);
		} else
			bubbleDown(k);
	}
